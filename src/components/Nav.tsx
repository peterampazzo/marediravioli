  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 overflow-visible transition-all duration-300 ${
        isScrolled
          ? "bg-[#FDFBF7]/96 backdrop-blur-md shadow-sm"
          : "bg-[#FDFBF7]"
      }`}
      style={{
        borderBottom: isScrolled ? "1px solid hsl(36 22% 86%)" : "none",
      }}
    >
      {/* ── DESKTOP ── */}
      <div className="hidden md:flex items-center h-20 relative container mx-auto px-8">
        {/* Left nav */}
        <nav
          aria-label="About Mare di Ravioli"
          className="flex items-center gap-8 flex-1"
        >
          <a href="#story" className={linkClass} data-testid="link-nav-story">
            About
          </a>
          <a
            href="#mission"
        ...
        >
          ...
          <a
            href="#pickup"
            className={`${linkClass} bg-[#F4C542] text-foreground px-5 py-2 rounded-full font-bold hover:bg-[#f0bc30] transition-colors`}
      ...
      {/* ── MOBILE ── */}
      <div className="flex md:hidden items-center h-16 relative px-4">
        <button
          ref={menuButtonRef}
          type="button"
      ...
      {isMobileMenuOpen && (
        <div
          ...
          className="md:hidden absolute top-full left-0 right-0 bg-[#FDFBF7] border-b border-[#e0d8cc] shadow-lg"
        >
          <nav
            aria-label="Mobile navigation"
            className="container mx-auto px-5 py-3 flex flex-col"
          >
            {[
